...
def main():
    ...
    if args.use_wandb:
        wandb.init(
            ...
            dir="/network/scratch/l/lavoiems/wandb/", # TODO, remove hard-coding of this. Need to use this to avoid out-of-memory
            name=run_name,
            config=vars(args),
    ...
    actor_kwargs = {
        ...
        "norm_beta": args.actor_norm_beta,
    }
    critic_kwargs = {
        "n_obs": n_critic_obs,
        "n_act": n_act,
        ...
        "device": device,
        "norm_beta": args.critic_norm_beta,
    }
    ...
    if args.agent == "fasttd3":
        if env_type in ["mtbench"]:
            ...
            if args.sim_type in ["vq_both", "vq_critic", "l1_both", "l1_critic", "l1_actor", "l2_both", "l2_critic", "l2_actor"]:
                from fast_td3 import TupleCritic as Critic
            else:
                from fast_td3 import Critic

            if args.sim_type in ["vq_both", "vq_actor", "norm_both", "norm_actor", "l1_both", "l1_actor", "l2_both", "l2_actor"]:
                from fast_td3 import TupleActor as Actor
            else:
    ...
    def evaluate():
        ...
        for i in range(eval_envs.max_episode_steps):
            with torch.no_grad(), autocast(
                ...
                actor_out = actor(obs)
                if type(actor_out) == tuple:
                    actions, diff, _ = actor_out
                else:
                    actions = actor_out
    ...
    def update_main(data, logs_dict):
        with autocast(
            ...
            actor_out = actor(next_observations)
            if type(actor_out) == tuple:
                actions_, _, _ = actor_out
            else:
                actions_ = actor_out
            next_state_actions = (actions_ + clipped_noise).clamp(
            ...
            if type(qf1) == tuple:
                qf1, diff1, _ = qf1
                logs_dict["critic_embeddings_loss"] = diff1.detach()
            if type(qf2) == tuple:
                qf2, diff2, _ = qf2
        ...
        scaler.scale(qf_loss + diff1 + diff2).backward()
    ...
    def update_pol(data, logs_dict):
        with autocast(
            ...
            actor_out = actor(data["observations"])
            if type(actor_out) == tuple:
                actions_, diff, _ = actor_out
                logs_dict["actor_embeddings_loss"] = diff.detach()
            else:
                actions_ = actor_out
                diff = 0
            qf1, qf2 = qnet(critic_observations, actions_)
            if type(qf1) == tuple:
                qf1, _, _ = qf1
            if type(qf2) == tuple:
                qf2, _, _ = qf2
        ...
        scaler.scale(actor_loss + diff).backward()
        ...
        if global_step > args.learning_starts:
            ...
            if (
                ...
                save_params(
                    ...
                    args,
                    f"/network/scratch/l/lavoiems/fastrl/models/{run_name}_{global_step}.pt",
                )
    ...
    save_params(
        ...
        critic_obs_normalizer,
        args,
        f"/network/scratch/l/lavoiems/fastrl/models/{run_name}_final.pt",
    )
